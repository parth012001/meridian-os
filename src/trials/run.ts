...
export function scorecard() {
  ...
  for (const r of rows) {
    ...
    for (const c of JSON.parse(r.checks) as CheckResult[]) { const cc = (b.checks[c.id] ??= { pass: 0, total: 0 }); cc.total++; if (c.pass) cc.pass++; else cc.lastFail ??= c.detail; }   // rows are newest-first: keep the latest failure
  }
